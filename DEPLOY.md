...
Ошибка значит: Nginx работает, но не может получить ответ от приложения (контейнер `app`). Что сделать по шагам:

1. **Проверить, что контейнеры запущены:**
   ```bash
   docker compose ps
   ```
   У сервисов `app` и `db` должно быть состояние `Up`. Если `app` постоянно перезапускается (Restarting) — приложение падает при старте.

2. **Посмотреть логи приложения** (там будет причина падения):
   ```bash
   docker compose logs app
   ```
   Частые причины:
   - **Ошибка подключения к БД** (например `ECONNREFUSED`, `password authentication failed`) — проверьте, что контейнер `db` запущен и в `.env` указаны правильные `DB_PASSWORD` и `DATABASE_URL` (в docker-compose подставляется `postgres://hookahpos:${DB_PASSWORD}@db:5432/hookahpos`).
   - **Отсутствует папка `public`** — приложение в production отдаёт статику из `public`; она создаётся при сборке образа (Dockerfile). Пересоберите: `docker compose up -d --build`.

3. **Проверить, что приложение слушает порт внутри контейнера:**
   ```bash
   docker compose exec app wget -qO- http://localhost:3001/ || true
   ```
   Если ответ пустой или «Connection refused» — приложение не поднялось; смотрите логи из шага 2.

4. **Перезапустить стек** (после правки `.env` или кода):
   ```bash
   docker compose down
   docker compose up -d --build
   docker compose logs -f app
...
В коде приложения добавлено ожидание БД при старте (несколько попыток подключения), поэтому временная недоступность PostgreSQL при подъёме контейнеров не должна сразу приводить к падению.

#### password authentication failed for user "hookahpos"

PostgreSQL выставляет пароль пользователю **только при первой инициализации** тома с данными. Если вы потом поменяли `DB_PASSWORD` в `.env`, в БД по-прежнему старый пароль — приложение подключается с новым и получает отказ.

**Вариант А: данные в БД не нужны (чистый старт)**

Остановить контейнеры и **удалить том с данными** — при следующем запуске Postgres создаст БД заново с паролем из текущего `.env`:

```bash
docker compose down -v
# Проверьте, что в .env нужный DB_PASSWORD
docker compose up -d
docker compose exec app node migrations/run.js
```

**Вариант Б: данные нужно сохранить**

Тогда пароль менять нельзя: в `.env` должен быть тот же `DB_PASSWORD`, с которым том создавался в первый раз. Если вы его не помните, остаётся только Вариант А (потеря данных) или сброс пароля внутри Postgres вручную (через `docker compose exec db psql -U hookahpos ...` и `ALTER USER`).

Сообщения про пользователя **"pgsql"** (Role "pgsql" does not exist) — это попытки подключения с другого сервиса или с хоста (например, старый клиент/скрипт). На работу приложения они не влияют; приложение использует пользователя `hookahpos`.
